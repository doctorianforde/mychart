...
export default function LabResultsManager({
  ...
  labUploadMessage, setLabUploadMessage, selectedPatient,
  ...
  if (userData?.role === 'staff' && !selectedPatient) {
    return (
      <div className="mb-8 p-4 sm:p-8 bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg border border-white/60">
        <h2 className="text-2xl sm:text-3xl font-bold mb-4 text-[#4A3A33] font-['Montserrat']">Lab Results</h2>
        <p className="text-[#4A3A33]/60">Select a patient above to view or upload lab results.</p>
      </div>
    );
  }
